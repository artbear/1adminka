...
Процедура ЗарегистрироватьКоманду(Знач ИмяКоманды, Знач Парсер) Экспорт
	...
	ОписаниеКоманды = Парсер.ОписаниеКоманды(ИмяКоманды, "Загрузить конфигурацию из хранилища 1C");
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, ПараметрыСистемы.ОбщиеПараметрыКоманд().КаталогХранилища, "Локальный путь к каталогу хранилища 1С. Например, C:\Storage1C");
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, ПараметрыСистемы.ОбщиеПараметрыКоманд().ПользовательХранилища, "Имя пользователя хранилища 1С");
	Парсер.ДобавитьИменованныйПараметрКоманды(ОписаниеКоманды, ПараметрыСистемы.ОбщиеПараметрыКоманд().ПарольХранилища, "Пароль пользователя хранилища 1С");
...
Функция ВыполнитьКоманду(Знач ПараметрыКоманды) Экспорт
	...
	// Конфигуратор.ПолучитьВерсиюИзХранилища(КаталогХранилища, ПользовательХранилища, ПарольХранилища);

	ЗагрузитьКонфигурациюИзФайла(ФайлВерсии);
	
	ПараметрыСистемы.Лог.Отладка(Конфигуратор.ВыводКоманды());
    Возврат 0;
КонецФункции
...
Функция ПолучитьВерсиюИзХранилища(Знач СтрокаСоединения)
	...
	Иначе
	    // Конфигуратор = ПараметрыСистемы.Конфигуратор(); 
		// Конфигуратор.ПолучитьВерсиюИзХранилища(мНастройки.ПутьКХранилищу, мНастройки.ПользовательХранилища, мНастройки.ПарольХранилища, мНастройки.НомерВерсии);
...
Процедура ЗагрузитьКонфигурациюИзФайла(Знач ФайлВерсии, Знач ОбновитьИБ = Истина)
	...
	ПараметрыСистемы.Конфигуратор().ЗагрузитьКонфигурациюИзФайла(ФайлВерсии, ОбновитьИБ);
КонецПроцедуры
...
Функция ИмяФайлаИсходниковХранилища(Знач ИмяФайлаПоУмолчанию = "")
	...
	Возврат ОбъединитьПути(ПараметрыСистемы.Конфигуратор().КаталогСборки(), ИмяФайлаПоУмолчанию);
КонецФункции

Функция ПодготовитьЛогTool1CD()
	ФайлЛогаTool1CD = Новый Файл(ПараметрыСистемы.Конфигуратор().КаталогСборки() + "\tool1cd_log.txt");
	Если ФайлЛогаTool1CD.Существует() Тогда
