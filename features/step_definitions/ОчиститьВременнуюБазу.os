...
// //в лог файле конфигуратора существует строка "Обновление конфигурации базы данных успешно завершено"
// Процедура ВЛогФайлеКонфигуратораСуществуетСтрока(Знач ПарамСтрока1) Экспорт
...
Процедура ЯДобавляюПользователяИПарольТестовогоХранилищаКакПараметрКомандыПродукта() Экспорт
	ЯДобавляюПараметрКомандыПродукта("-storage-user Администратор");
	// ЯДобавляюПараметрКомандыПродукта("-storage-user Администратор -storage-password """" ");
КонецПроцедуры
...
Процедура ЯДобавляюКаталогСборкиКакПараметрКомандыПродукта() Экспорт
	...
	// 	ПараметрыКоманды = "";
	// КонецЕсли;
	// ПараметрыКоманды = СтрШаблон("%1 -dir %2", ПараметрыКоманды, КаталогСборки()); 
...
Процедура ЯДобавляюКаталогТестовогоХранилищаКакПараметрКомандыПродукта() Экспорт
	ЯДобавляюПараметрКомандыПродукта("-storage-dir " + КаталогСборки());
	// ЯДобавляюПараметрКомандыПродукта("-storage-dir "+ОбъединитьПути(КаталогТестовыхФикстур(), "storage_initial.1CD"));
КонецПроцедуры
